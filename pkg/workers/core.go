...
func main() {
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				if err := sendHelloMessage(); err != nil {
					return cli.NewExitError(err.Error(), 1)
				}
				return nil
			},
...
func worker(tag string, concurrency int) error {
	consumerTag := tag
...
func sendHelloMessage() error {
	...
	asyncResult, err := server.SendTaskWithContext(ctx, &helloTask)
	if err != nil {
